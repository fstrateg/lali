...
class SMSElectro
{
    ...
    private static function getClientsForReminder($day)
    {
        if ($day==30)
        {
            $exc='electro_except_30';
            $col='electro_30';
            $ds=SettingsRecord::findValue('sms','electro30');
        }
        else
        {
            $exc='electro_except_60';
            $col='electro_60';
            $ds=SettingsRecord::findValue('sms','electro60');
        }
        $day=$day-1; //поправка чтобы разбежаться с лазеркой
